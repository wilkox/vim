" Filetype plugin for editing tidy files.
" Version 2011-11-02 from http://vim.wikia.com/wiki/tidy
...
" Highlight a column in tidy text.
" :Tidy 1    " highlight first column
" :Tidy 12   " highlight twelfth column
" :Tidy 0    " switch off highlight
function! TIDYH(colnr)
  if a:colnr > 1
    let n = a:colnr - 1
...
command! -nargs=1 Tidy :call TIDYH(<args>)
...
function! s:Delimiter(delim)
  if !empty(a:delim)
    ...
    let b:tidy_delimiter = want
  endif
  let b:changed_done = -1
  let b:tidy_column = 1
  silent call s:Highlight(b:tidy_column)
  echo printf('Delimiter = "%s"', b:tidy_delimiter == "\t" ? '\t' : strtrans(b:tidy_delimiter))
endfunction
command! -buffer -nargs=? Delimiter call s:Delimiter('<args>')
 
" Get string containing delimiter (default ',') specified for current buffer.
" A command like ':let g:tidy_delimiter = ";"' changes the default.
...
function! s:GetStr(id)
  if !exists('b:tidy_delimiter') || empty(b:tidy_delimiter)
    if exists('g:tidy_delimiter') && !empty(g:tidy_delimiter)
      let b:tidy_delimiter = g:tidy_delimiter
    else
      let b:tidy_delimiter = ','
    endif
  endif
  if !exists('b:tidy_str')
    let b:tidy_str = {'delim': ''}
  endif
  if b:tidy_str['delim'] !=# b:tidy_delimiter
    " Define strings using delimiter ',' then substitute if required.
    let b:tidy_str['delim'] = ','
    let b:tidy_str['numco'] = '\%(\%("\%([^"]\|""\)*"\)\|\%([^,"]*\)\)'
    let b:tidy_str['expr1'] = '\%(\%("\zs\%([^"]\|""\)*\ze"\)\|\%(\zs[^,"]*\ze\)\)'
    let b:tidy_str['expr2'] = '\%(\%(\zs"\%([^"]\|""\)*",\?\ze\)\|\%(\zs[^,"]*,\?\ze\)\)'
    let b:tidy_str['expr3'] = '^\%(\%(\%("\%([^"]\|""\)*"\)\|\%([^,"]*\)\),\)\{'
    let b:tidy_str['delco'] = ',$'
    let b:tidy_str['sear1'] = '^\%(\%("\%([^,]\|""\)*\zs'
    let b:tidy_str['sear2'] = '\ze\%([^,]\|""\)*"\)\|\%([^,"]*\zs'
    let b:tidy_str['sear3'] = '\ze[^,"]*\)\)'
    let b:tidy_str['sear4'] = '^\%(\%(\%("\%([^"]\|""\)*"\)\|\%([^,"]*\)\),\)\{'
    let b:tidy_str['sear5'] = '}\%(\%("\%([^,]\|""\)*\zs'
    if b:tidy_delimiter != ','
      let d1 = b:tidy_delimiter  " d1 replaces ',' in '[...]'
      ...
      for key in keys(b:tidy_str)
        if key == 'delim'
          let b:tidy_str[key] = b:tidy_delimiter
        else
          let replace = substitute(b:tidy_str[key], '\[[^]]*\zs,', d1, 'g')
          let b:tidy_str[key] = substitute(replace, ',', d2, 'g')
        endif
      endfor
    endif
  endif
  return b:tidy_str[a:id]
endfunction
 
" Get the number of columns
" Optionally takes one parameter, a specified line number
function! s:GetNumCols(...)
  let b:tidy_max_col = 1
  ...
  if a:0 == 1
   ...
   " lines; at least one of them should contain typical tidy data
  else
  ...
  for l in rg_cols_to_check
    ...
    if b:tidy_max_col < c
      let b:tidy_max_col = c
    endif
  endfor
  if b:tidy_max_col <= 1
    let b:tidy_max_col = 999
    call s:Warn('No delimiter-separated columns were detected')
  endif
  return b:tidy_max_col
endfunction
...
let b:tidy_heading_line_number=1
 
" Extract and echo the column header on the status line.
function! s:PrintColumnInfo(colnr)
  let colHeading = substitute(matchstr(getline(b:tidy_heading_line_number), s:GetExpr(a:colnr)),
    \ '^\s*\(.*\)\s*$', '\1', '')
...
" Change tidy_heading_line_number to specified line.
" If no line is specified, use the current line.
function! s:SetHeadinglineNumber(linenr)
  if empty(a:linenr)
    let b:tidy_heading_line_number = line('.')
  else
    let b:tidy_heading_line_number = a:linenr
  endif
  " Update the displayed column name to use the new heading line
  call s:PrintColumnInfo(b:tidy_column)
  " Update the maximum number of columns based on heading
  " line
  call s:GetNumCols(b:tidy_heading_line_number)
endfunction
...
function! s:Highlight(colnr)
  silent! call matchdelete(b:tidy_match)
  if a:colnr > 0
    if exists('*matchadd')
      let b:tidy_match = matchadd('Keyword', s:GetExpr(a:colnr), -1)
    else
...
function! s:HighlightNextColumn()
  if b:tidy_column < b:tidy_max_col
    let b:tidy_column += 1
  endif
  call s:Highlight(b:tidy_column)
endfunction
...
let b:undo_ftplugin = "setlocal wrap< textwidth<"
    \ . "|if exists('*matchdelete')|call matchdelete(b:tidy_match)|else|2match none|endif"
...
function! s:CompareLines(line1, line2)
  let val1 = s:GetValue(a:line1, b:tidy_sort_regex)
  let val2 = s:GetValue(a:line2, b:tidy_sort_regex)
  if type(val1) != type(val2)
    let val1 = type(val1)
    let val2 = type(val2)
  endif
  let ascending = val1 > val2 ? 1 : val1 < val2 ? -1 : 0
  return b:tidy_sort_ascending ? ascending : -ascending
endfunction
...
function! s:SortColumn(bang, range_given, line1, line2, ...) range
  let colnr = b:tidy_column
  let args = copy(a:000)
  if len(args) > 0 && args[0] =~ '^\d\+$'
    let colnr = str2nr(args[0])
    unlet args[0]
  endif
  if colnr < 1 || colnr > b:tidy_max_col
    call s:Warn('Column number out of range')
  endif
 
  "First check that the headings line is a valid line
  if b:tidy_heading_line_number >= line('$')
    call s:Warn('No lines to sort - specified heading line ['.b:tidy_heading_line_number.
  ...
  if a:range_given
    ...
    let first = b:tidy_heading_line_number + 1
    let last = line('$')
  endif
 
  let flags = join(args)
  if flags == 'f'
    let b:tidy_sort_ascending = empty(a:bang)
    let b:tidy_sort_regex = s:GetExpr(colnr)
...
function! s:CopyColumn(args)
  ...
  let col = empty(l[1]) ? b:tidy_column : str2nr(l[1])
  let reg = empty(l[2]) ? '@' : l[2]
  if col < 1 || col > b:tidy_max_col
    call s:Warn('Column number out of range')
    return
  endif
...
nnoremap <silent> <buffer> J <Down>:call <SID>Focus_Column(b:tidy_column)<CR>
nnoremap <silent> <buffer> K <Up>:call <SID>Focus_Column(b:tidy_column)<CR>
nnoremap <silent> <buffer> <C-f> <PageDown>:call <SID>Focus_Column(b:tidy_column)<CR>
nnoremap <silent> <buffer> <C-b> <PageUp>:call <SID>Focus_Column(b:tidy_column)<CR>
nnoremap <silent> <buffer> <C-d> <C-d>:call <SID>Focus_Column(b:tidy_column)<CR>
nnoremap <silent> <buffer> <C-u> <C-u>:call <SID>Focus_Column(b:tidy_column)<CR>
nnoremap <silent> <buffer> 0 :let b:tidy_column=1<CR>:call <SID>Highlight(b:tidy_column)<CR>
nnoremap <silent> <buffer> $ :let b:tidy_column=b:tidy_max_col<CR>:call <SID>Highlight(b:tidy_column)<CR>
nnoremap <silent> <buffer> gm :call <SID>Focus_Column(b:tidy_column)<CR>
...
execute 'augroup tidy' . bufnr('')
  ...
  autocmd BufEnter <buffer> silent call s:Highlight(b:tidy_column)
augroup END
